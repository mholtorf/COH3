...
events <- 
  ...
  mutate(description = gsub("\n", "<br>", description),
         dtstart = lubridate::with_tz(dtstart, "America/Los_Angeles"),
         ...
         daysuntill = as.numeric(difftime(as.Date(dtstart), as.Date(Sys.Date()), units = "days"))) %>%
  arrange(dtstart)
...
updates <- 
  ...
  filter(dtstart > Sys.Date(),
         daysuntill < 2 &
         difftime(last.modified, Sys.Date(), units = "days") > -10)
